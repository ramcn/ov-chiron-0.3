...
def train():
    ...
    x = tf.placeholder(tf.float32,shape = [FLAGS.batch_size,FLAGS.sequence_len], name='input1')
    ...
    saver = tf.train.import_meta_graph('./res50/final.ckpt-10.meta', clear_devices=True)
    graph = tf.get_default_graph()
    input_graph_def = graph.as_graph_def()
    sess1 = tf.Session()
    saver.restore(sess1, "./res50/final.ckpt-10")
